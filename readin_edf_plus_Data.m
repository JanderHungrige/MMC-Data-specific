...
for i=1:size(edfFiles(:,1),1) % the edf files are split into several files
    [data, header] = ReadEDF(edfFiles(i).name); % Load EDF file
    disp(edfFiles(i).name)
    Resp_idx = find(strcmp(data.label, 'Chest')); % find the index of certain signal
    ECG_idx = find(strcmp(data.label, 'Pulse')); % find the index of certain signal
    EOG_idx = find(strcmp(data.label, 'EEGT5T6')); % find the index of certain signal
    Chin_idx = find(strcmp(data.label, 'EEGA2Ref')); % find the index of certain signal
    EMG_idx = find(strcmp(data.label, 'EEGP4Ref')); % find the index of certain signal


    % Signal artificailly elonginated. Cut it again to find correct FS
    zResp=find(diff(header(Resp_idx,:),40)==0); %find where the signal is set to ero
    zEMG=find(diff(header(EMG_idx,:),40)==0); %find where the signal is set to zero
    zEOG=find(diff(header(EOG_idx,:),40)==0); %find where the signal is set to zero
    zChin=find(diff(header(Chin_idx,:),40)==0); %find where the signal is set to zero
    ...
    ECG_parts{i}=header(ECG_idx,:); % load ECG
    Resp_parts{i}=(header(Resp_idx,1:zResp)); %Load Respiration
    EMG_parts{i} =(header(EMG_idx,1:zEMG));%Load EMG
    EOG_parts{i} =(header(EOG_idx,1:zEMG));%Load EOG
    Chin_parts{i}=(header(Chin_idx,1:zChin));%Laod Chin
...
offset_Resp=(length(header(ECG_idx,:))-length(Resp))/500; % if neg resp is delayed. If pos. ECG is delayed
offset_EMG=(length(header(EMG_idx,:))-length(EMG))/500; % if neg resp is delayed. If pos. ECG is delayed
offset_EOG=(length(header(ECG_idx,:))-length(EOG))/500; % if neg EOG is delayed. If pos. ECG is delayed
offset_Chin=(length(header(EMG_idx,:))-length(Chin))/500; % if neg Chin is delayed. If pos. ECG is delayed
...
if plotreadin
    ...
    ax1= subplot(5,1,1);plot(tECG,header(ECG_idx,:));ylabel('ECG');
